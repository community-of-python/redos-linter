...
class TestMainFunction:
    ...
    def test_single_vulnerable_file(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
...
""")
    ...
    def test_mixed_safe_and_vulnerable(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
...
""")
    ...
    def test_directory_scan(
        ...
        (tmp_path / "node_modules" / "should_be_ignored.py").write_text(
            "import re\nr3 = re.compile(r'ignored')"
        )
        (tmp_path / ".venv").mkdir()
        (tmp_path / ".venv" / "also_ignored.py").write_text(
    ...
    def test_color_output_disabled(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
